...
def najdi_figure(bitb):
    return [
        koord_v_bit(x, y) for y in range(8) for x in range(8)
...
class Plosca:
    ...
    def zabelezi_potezo(self, poteza):
        ...
        if fig == (TIPI_FIGUR["w_r"] if self.barva else TIPI_FIGUR["b_r"]):
            ...
            if poteza.od == koord_v_bit(7, 0 if self.barva else 7):
                if self.pravice_rokade[0 if self.barva else 2]:
                    poteza.spremeni_pravice_rokade[0
                                                   if self.barva else 2] = True
                self.pravice_rokade[0 if self.barva else 2] = False
            # Damina stran
        ...
        if poteza.je_zajem():
            ...
            if poteza.je_en_passant():
                ...
                zajeti_poz = (koord_premik(poteza.do, 0, -1)
                              if self.barva == BELI else koord_premik(
                                  poteza.do, 0, 1))
            ...
            self.figure = [
                b_and(bitb, b_not(zajeti_poz))
                if je_v_bitb(bitb, zajeti_poz) else bitb
        ...
        if poteza.je_promocija():
            ...
            self.figure = [
                b_or(b_and(bitb, b_not(poteza.od)), poteza.do)
                if je_v_bitb(bitb, poteza.od) else b_or(
                    b_and(bitb, b_not(trdnjava_poz_od)), trdnjava_poz_do)
                if je_v_bitb(bitb, trdnjava_poz_od) else bitb
                for bitb in self.figure
            ]
        else:
            self.figure = [
                b_or(b_and(bitb, b_not(poteza.od)), poteza.do)
                if je_v_bitb(bitb, poteza.od) else bitb for bitb in self.figure
            ]
    ...
    def razveljavi_potezo(self):
        ...
        if poteza.je_zajem():
            ...
            if poteza.je_en_passant():
                ...
                # CRNI: Zajeti kmet je 1 gor nad destinacijo kmeta
                zajeti_poz = (koord_premik(poteza.do, 0, -1)
                              if barva == BELI else koord_premik(
                                  poteza.do, 0, 1))
            self.figure[poteza.tip_zajete_fig] = b_or(
                self.figure[poteza.tip_zajete_fig], zajeti_poz)

        if poteza.je_promocija():
            self.figure[poteza.figura_promocije()] = b_and(
                self.figure[poteza.figura_promocije()], b_not(pozicija.do))
            ...
            if poteza.flags == ROKADA_Q:
                ...
                self.figure = [
                    b_or(b_and(bitb, b_not(poteza.do)), poteza.od)
                    if je_v_bitb(bitb, poteza.do) else b_or(
                        b_and(bitb, b_not(trdnjava_poz_do)), trdnjava_poz_od)
                    if je_v_bitb(bitb, trdnjava_poz_do) else bitb
                    for bitb in self.figure
                ]
        else:
            self.figure = [
                b_or(b_and(bitb, b_not(poteza.do)), poteza.od)
                if je_v_bitb(bitb, poteza.do) else bitb for bitb in self.figure
            ]
        self.barva = not self.barva
    ...
    def gen_skakac(self, barva):
        list_potez = []
        poz_list = najdi_figure(
            self.figure[TIPI_FIGUR["w_n" if barva else "b_n"]])
        for poz in poz_list:
            for x in (-2, -1, 1, 2):
                for y in (-2, -1, 1, 2):
                    premik = koord_premik(poz, x, y)
                    # Nista oba premik za 1
                    if (not (x in (-1, 1) and y in (-1, 1))
                            and not (x in (-2, 2) and y in (-2, 2))
                            and premik != None):
    ...
    def gen_kmet_potisk(self, barva):
        list_potez = []
        poz_list = najdi_figure(
            self.figure[TIPI_FIGUR["w_p" if barva else "b_p"]])
        for poz in poz_list:
            # 1 gor
    ...
    def gen_legalne_poteze(self):
        ...
        potencialne_poteze = [
            self.prilagodi_potezo(poteza) for poteza in potencialne_poteze
        ]

        # K
        poz_list = najdi_figure(self.figure[TIPI_FIGUR["w_k"]] if self.barva
        ...
        self.figure = [
            odstrani_iz_bitb(bitb, pin_bitb) for bitb in self.figure
        ]
        for poz in pin_poz:
            # Ray cast od kralja
        ...
        for pot in potencialne_poteze:
            if pot.je_en_passant():
                ...
                if (je_v_bitb(ray_cast_nasp, pot.poz_zajete_fig)
                        and je_v_bitb(ray_cast_kralj_bitb, pot.od)):
                    neveljavne_pot.append(pot)
                if (je_v_bitb(ray_cast_nasp, pot.od) and je_v_bitb(
                        ray_cast_kralj_bitb, pot.poz_zajete_fig)):
                    neveljavne_pot.append(pot)
        ...
        potencialne_poteze = [
            pot for pot in potencialne_poteze if pot not in neveljavne_pot
        ]
        ...
        if self.pravice_rokade[0 if self.barva else 2]:
            ...
            if (self.figura_na_poz(poz_1) == None
                    and self.figura_na_poz(poz_2) == None):
                # Preverimo, če sta poziciji napadeni
                if (not je_v_bitb(self.napadeni, poz_1)
                        and not je_v_bitb(self.napadeni, poz_2)):
                    self.legalne_poteze.append(
                        self.prilagodi_potezo(Poteza(poz_kralj, poz_2)))
        # Rokada - damina stran
        if self.pravice_rokade[1 if self.barva else 3]:
            poz_1 = koord_premik(poz_kralj, -1, 0)
            poz_2 = koord_premik(poz_kralj, -2, 0)
        ...
        for poteza in potencialne_poteze:
            if poteza.od == poz_kralj:
                ...
                continue
            if poteza.je_zajem():
                if poteza.tip_zajete_fig in range(0 if self.barva else 6, 6
                                                  if self.barva else 12):
                    # Spustimo poteze, ki so zajemi svoje barve
                    continue
            if (self.figura_na_poz(poteza.od) == (TIPI_FIGUR["w_p"]
                                                  if self.barva else
                                                  TIPI_FIGUR["b_p"])):
                if not poteza.je_en_passant():
                    if self.figura_na_poz(poteza.do) == None:
                        if poteza.do not in (koord_premik(
                                poteza.od, 0, 1 if self.barva else -1),
                                             koord_premik(
                                                 poteza.od, 0, 2
                                                 if self.barva else -2)):
